...
- 交易胜率、平均盈亏、盈亏比、盈利因子、最佳与最差单日
- 做多/做空笔数、净收益、胜率、平均盈利与平均亏损对比
- 权益口径与交易口径对账，以及回撤、集中度、保证金和强平风险
- Polymarket 普通仓位与组合单均同步当前/历史数据，市场标题通过百度 LLM 翻译为简体中文并持久化复用
...
每轮同步后自动重建并持久化收益汇总，页面直接读取；每天北京时间 00:05 再全量校准。
资产总览、风险、对账、同步状态、数据完整性和四个周期的交易行为分析也使用可重建读取
模型；财务总计按北京时间每日、账户和类型预聚合，分页明细仍直接读取事实表，因此筛选
与导出不会丢失原始记录。
...
| 账户余额 | 60 秒 |
| 账务历史 | 5 分钟 |
| 已平仓仓位 | 60 秒；检测到仓位减少时立即补拉 |
| 同步健康状态 | 60 秒 |
| 浏览器页面刷新 | 60 秒 |

四类后端同步流按各自周期独立执行：15 秒调度心跳不会重复请求尚未到期的余额、历史或
平仓接口。检测到同一方向的仓位数量减少或消失时会立即补拉平仓记录；已平仓与账务历史
按上次成功游标增量拉取，并保留 5 分钟重叠窗口用于幂等补漏；Polymarket 因结算记录可能
延迟进入官方接口，固定回看最近 72 小时。仍持有的长周期仓位会扩大平仓查询起点，避免
跨越常规窗口后漏记。

当前账户与资产余额会覆盖写入两张最新状态表，网页无需扫描历史快照；组合净值曲线仍按
5 分钟采样。账户、资产和持仓明细快照每天首次成功同步时各保留一份，历史仓位、费用、
资金费和财务流水不受影响；每日边界采用北京时间（Asia/Shanghai）00:00。同步周期可在
`.env` 调整。同步任务运行记录默认保留 7 天，业务数据库默认永久保留；备份默认保留
90 天，日志建议保留 7 天。旧版 UTC 日快照会在升级后一次性转换为北京时间自然日。
所有读取模型都可由事实表重新生成；过期或缺失时接口自动回退到事实表，不会返回陈旧缓存。
...
- [交易所 API 参考](docs/exchange-api-reference.md)
- [交易所接入问题复盘与工程规范](docs/exchange-integration-lessons.md)
